/**
 * Comprehensive World Countries and International Calling Codes dataset
 * Covers all sovereign nations and territories recognized worldwide.
 */
...
export const WORLD_COUNTRIES = [
  ...
  { code: 'VI', name: 'Virgin Islands (US)', dialCode: '+1340', flag: '🇻🇮' },
...
// Frequent / Primary countries for fast selection
export const POPULAR_COUNTRY_CODES = [
  'EG', 'SA', 'AE', 'KW', 'QA', 'OM', 'BH',
  'BG', 'ES', 'GB', 'US', 'FR', 'DE', 'IT',
  'TR', 'JO', 'LB', 'MA', 'DZ', 'TN', 'IQ'
];

export const POPULAR_COUNTRIES = POPULAR_COUNTRY_CODES
  .map(code => WORLD_COUNTRIES.find(c => c.code === code))
  .filter(Boolean);

export const findCountryByCode = (code) => {
  if (!code) return null;
  const upper = String(code).toUpperCase().trim();
  return WORLD_COUNTRIES.find(c => c.code === upper) || null;
};

export const findCountryByDialCode = (dial) => {
  if (!dial) return null;
  const clean = dial.startsWith('+') ? dial : `+${dial}`;
  return WORLD_COUNTRIES.find(c => c.dialCode === clean) || null;
};
