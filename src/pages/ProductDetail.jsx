    ...
    <div className="min-h-screen bg-transparent text-[#F3E6D0] pt-28 sm:pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 text-xs text-[#D8BE99] mb-8 font-cinzel uppercase tracking-wider">
          ...
          <ChevronRight className="w-3 h-3 rtl:rotate-180" />
          <span className="text-[#F3E6D0] truncate max-w-xs">{displayName}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-20">
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            {/* Main Stage */}
            <div className="relative aspect-[3/4] bg-gradient-to-b from-[#0B0A08] via-[#0B0A08] to-[#0B0A08] border border-[#D4AF37]/25 p-8 sm:p-12 flex items-center justify-center overflow-hidden shadow-2xl">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2">
                {galleryImages.map((img, idx) => (
                  <button
                    ...
                    className={`h-20 w-20 flex-shrink-0 bg-[#0B0A08] border p-2 flex items-center justify-center transition-all ${
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div>
              ...
              {/* Rating Summary */}
              <div className="flex items-center gap-3 text-xs text-[#D8BE99]">
                <div className="flex gap-1 text-[#D4AF37]">
                  {[...Array(5)].map((_, i) => <Star key={i} className="w-3.5 h-3.5 fill-current" />)}
                </div>
                <span className="text-[#F3E6D0] font-bold">{product.rating || '5.0'}</span>
                <span>•</span>
            ...
            <div className="py-4 border-y border-[#D4AF37]/15 flex items-baseline gap-4">
              ...
              <span className="text-xs text-[#D8BE99] uppercase tracking-wider">
            ...
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest text-[#D8BE99] font-cinzel flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center border border-[#D4AF37]/30 bg-black/50 rounded-xs">
                  <button
                    ...
                    className="p-3 text-[#F3E6D0] hover:text-[#D4AF37] transition-colors"
                  >
            ...
            <div className="p-4 bg-[#0B0A08] border border-white/10 rounded space-y-2.5 text-xs text-[#D8BE99]">
        ...
        <div className="border-t border-[#D4AF37]/20 pt-12 mb-20">
            ...
            <div className="flex gap-8 text-xs uppercase font-cinzel tracking-[0.25em]">
              <button
                ...
                className={`pb-3 border-b-2 transition-colors ${
                  activeTab === 'pyramid' ? 'border-[#D4AF37] text-[#D4AF37] font-bold' : 'border-transparent text-[#D8BE99]'
                }`}
              >
                Olfactory Pyramid
              </button>
              <button
                ...
                className={`pb-3 border-b-2 transition-colors ${
                  activeTab === 'performance' ? 'border-[#D4AF37] text-[#D4AF37] font-bold' : 'border-transparent text-[#D8BE99]'
                }`}
              >
                Performance Profile
              </button>
              <button
                ...
                className={`pb-3 border-b-2 transition-colors ${
                  activeTab === 'reviews' ? 'border-[#D4AF37] text-[#D4AF37] font-bold' : 'border-transparent text-[#D8BE99]'
                }`}
              >
          ...
          {activeTab === 'pyramid' && (
            <div className="max-w-4xl mx-auto space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                ...
                {/* Top Notes */}
                <div className="p-6 bg-[#0B0A08] border border-[#D4AF37]/20 space-y-3">
                  <div className="text-[11px] uppercase tracking-widest text-[#D4AF37] font-cinzel font-bold">
                    Top Notes (Opening)
                  </div>
                  <ul className="space-y-1.5 text-xs text-[#F3E6D0]">
                    {product.notes?.top?.map((n, i) => <li key={i}>{n}</li>) || <li>Add fragrance notes</li>}
                  </ul>
                </div>

                {/* Heart Notes */}
                <div className="p-6 bg-[#0B0A08] border border-[#D4AF37]/30 space-y-3 shadow-lg">
                  <div className="text-[11px] uppercase tracking-widest text-[#D4AF37] font-cinzel font-bold">
          ...
          {activeTab === 'performance' && (
            <div className="max-w-3xl mx-auto bg-[#0B0A08] border border-[#D4AF37]/20 p-8 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs">
                <div>
                  <span className="text-[#D8BE99] uppercase tracking-wider block mb-1">Longevity</span>
                  <span className="font-semibold text-base text-[#D4AF37]">{product.longevity || '10-12 Hours'}</span>
                </div>
                <div>
                  <span className="text-[#D8BE99] uppercase tracking-wider block mb-1">Sillage / Projection</span>
                  <span className="font-semibold text-base text-[#F3E6D0]">{product.sillage || 'Strong & Sophisticated'}</span>
                </div>
                <div>
                  <span className="text-[#D8BE99] uppercase tracking-wider block mb-1">Ideal Season</span>
                  <span className="font-semibold text-[#F3E6D0]">{product.season?.join(', ') || 'All Seasons'}</span>
                </div>
                <div>
                  <span className="text-[#D8BE99] uppercase tracking-wider block mb-1">Recommended Occasion</span>
                  <span className="font-semibold text-[#F3E6D0]">{product.occasion?.join(', ') || 'Daily Luxury, Gala'}</span>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="space-y-4">
                {product.reviews && product.reviews.length > 0 ? (
                  product.reviews.map((rev) => (
                    <div key={rev.id} className="p-6 bg-[#0B0A08] border border-white/10 space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="font-cinzel font-bold text-xs text-[#F3E6D0]">{rev.author}</span>
                      ...
                      <p className="text-xs text-[#D8BE99]">{rev.comment}</p>
                    </div>
                  ))
                ) : (
                  <p className="text-xs text-center text-[#D8BE99] py-6">No patron reviews yet. Be the first to share your olfactory impression.</p>
                )}
              </div>

              {/* Review Submission Form */}
              <div className="p-6 bg-[#0B0A08] border border-[#D4AF37]/30 space-y-4">
                ...
                <form onSubmit={handleReviewSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input
                      ...
                      className="bg-black/60 border border-white/10 px-3 py-2 text-xs rounded text-[#F3E6D0] focus:border-[#D4AF37] focus:outline-none"
                    />
                    <input
                      type="text"
