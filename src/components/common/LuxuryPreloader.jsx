...
export default function LuxuryPreloader({ onComplete }) {
  ...
  return (
    <div
      ref={curtainRef}
      className="fixed inset-0 z-[10000] bg-[#0B0A08] flex flex-col items-center justify-center select-none overflow-hidden"
    >
      ...
      <div ref={contentRef} className="relative z-10 flex flex-col items-center text-center space-y-6 px-6">
        ...
        <div ref={logoRef} className="space-y-2">
          <div className="text-4xl sm:text-5xl font-arabic font-bold text-[#F3E6D0] tracking-wide">
            سَـراب
          </div>
          <div className="text-[11px] font-sans font-extrabold uppercase tracking-[0.35em] text-[#D4AF37]">
            SARAB HAUTE PARFUMERIE
          </div>
          <div className="text-[9px] font-sans text-[#D8BE99] tracking-[0.25em] uppercase">
            PARIS • RIYADH • DUBAI
          </div>
        </div>
        ...
        <div className="w-48 sm:w-64 h-[1.5px] bg-white/[0.08] relative overflow-hidden rounded-full mt-4">
          <div
            className="h-full bg-gradient-to-r from-[#D8BE99] via-[#D4AF37] to-[#F2D675] transition-all duration-100 ease-out"
            style={{ width: `${progress}%` }}
          />
        </div>

        {/* Counter & Status */}
        <div className="flex items-center justify-between w-48 sm:w-64 text-[10px] font-mono text-[#D8BE99] tracking-widest pt-1">
